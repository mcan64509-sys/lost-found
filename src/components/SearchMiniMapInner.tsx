...
function RecenterMiniMap({ lat, lng }: { lat: number; lng: number }) {
  const map = useMap();
  useEffect(() => {
...
export default function SearchMiniMapInner({ lat, lng, radiusKm, items = [] }: Props) {
  return (
    <MapContainer
      center={[lat, lng]}
      zoom={11}
    ...
    >
      ...
      <RecenterMiniMap lat={lat} lng={lng} />
