  ...
  async function handleAiDescSuggest() {
    ...
    try {
      ...
      const res = await fetch("/api/ai/suggest-description", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title, category: category === "Diğer" ? customCategory || "Diğer" : category, type: "lost" }),
      });
      const data = await res.json();
    ...
    <>
      ...
      {duplicateWarning.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="w-full max-w-sm rounded-2xl border border-amber-500/30 bg-[#0d1a2e] p-6 shadow-2xl">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 rounded-xl bg-amber-500 py-2.5 text-sm font-bold text-slate-950 hover:bg-amber-400 transition"
      ...
      <main className="min-h-screen bg-slate-950 text-white">
        <div className="mx-auto max-w-2xl px-4 py-10">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h1 className="text-2xl font-black text-white">Kayıp İlanı Oluştur</h1>
              <p className="text-sm text-slate-500 mt-0.5">Kaybettiğin eşyayı adım adım tanımla</p>
            </div>
          </div>
            ...
            {STEPS.map((s, idx) => (
              <div key={s.id} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center gap-1.5">
                  ...
                  <span className={`text-[11px] font-semibold whitespace-nowrap ${step === s.id ? "text-amber-400" : step > s.id ? "text-slate-400" : "text-slate-600"}`}>
                    {s.label}
                  </span>
                </div>
                {idx < STEPS.length - 1 && (
                  <div className={`flex-1 h-px mx-2 mb-5 transition-colors ${step > s.id ? "bg-amber-500/50" : "bg-slate-800"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="space-y-5 rounded-3xl border border-slate-800 bg-slate-900/60 p-7">
              <div className="mb-6">
                ...
                <p className="text-sm text-slate-500 mt-1">Kaybolan eşyanı mümkün olduğunca detaylı tarif et. Bu bilgiler AI eşleştirmede kullanılır.</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Örn: Siyah iPhone 15 Pro"
                  ...
                  autoFocus
                />
                <p className="mt-1.5 text-xs text-slate-600">Marka, model veya renk gibi ayırt edici özellikler ekle</p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Eşyanın özellikleri, kaybedilme koşulları, üzerindeki özel işaretler vb. detaylı yaz. Bu bilgiler bulunmasını kolaylaştırır."
                  rows={5}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white placeholder:text-slate-600 outline-none focus:border-pink-500/50"
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Örn: 2 yaşında, orta boy"
                        className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white placeholder:text-slate-600 outline-none focus:border-pink-500/50"
                      />
                    </div>
                    <div>
          ...
          {step === 2 && (
            <div className="space-y-5 rounded-3xl border border-slate-800 bg-slate-900/60 p-7">
              <div className="mb-6">
                <h2 className="text-lg font-bold text-white">Nerede ve ne zaman?</h2>
                <p className="text-sm text-slate-500 mt-1">Kayboluş yeri ve tarihi harita aramasında kullanılır.</p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-amber-400">{category === "Diğer" ? customCategory || "Diğer" : category}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-7">
              <div className="mb-6">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Görseller bulunan kişinin eşyayı tanımasını sağlar. En az 1, en fazla 5 görsel yükle.
                </p>
              </div>
              ...
              {selectedImages.length < 5 && (
                <label className={`flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed p-10 cursor-pointer transition ${
                  previewUrls.length === 0
                    ? "border-slate-700 hover:border-amber-500/40 hover:bg-amber-500/5"
              ...
              <div className="mt-6 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
                <p className="text-xs font-semibold text-amber-400 uppercase tracking-wider mb-3">İlan Özeti</p>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-between mt-6">
            ...
            {step < 3 ? (
              <button
                ...
                disabled={!canGoNext()}
                className="flex items-center gap-2 rounded-xl bg-amber-500 px-6 py-3 text-sm font-bold text-slate-950 hover:bg-amber-400 transition disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Devam Et
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-amber-500 px-8 py-3 text-sm font-bold text-slate-950 hover:bg-amber-400 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitting ? (
                  <>
