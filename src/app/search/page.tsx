...
// DB category values (always Turkish — stored in DB as-is)
const CATEGORIES_DB = [
...
function SearchPageContent() {
  ...
  const syncUrl = useCallback((params: Record<string, string>) => {
    ...
    Object.entries(params).forEach(([k, v]) => {
      if (v && v !== "all" && v !== CATEGORIES_DB[0] && v !== "newest") {
        current.set(k, v);
      } else {
        current.delete(k);
      }
  ...
  const filteredItems = useMemo(() => {
    ...
    const filtered = allItems.filter((item) => {
      ...
      if (category !== CATEGORIES_DB[0] && item.category !== category) return false;
      if (keyword.trim()) {
      ...
      <main className="min-h-screen bg-slate-950 text-white animate-fade-in">
          ...
          <div className="mx-auto max-w-7xl px-4 md:px-6 py-8">
            <div className="flex flex-col md:flex-row md:items-center gap-4">
              <div className="flex-1">
                <h1 className="text-2xl font-black text-white">{t.search.title}</h1>
                <p className="text-sm text-slate-500 mt-1">
                  {loading ? t.common.loading : `${filteredItems.length} ${t.search.results}`}
            ...
            {searchHistory.length > 0 && !keyword && (
              <div className="flex flex-wrap items-center gap-2 mt-3">
                <span className="text-[11px] text-slate-600 uppercase tracking-wider">🕐</span>
                {searchHistory.map((h) => (
            ...
            {showFilters && (
              <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {/* Kategori */}
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">{t.search.category}</label>
                    <div className="relative">
                      ...
                      >
                        {CATEGORIES_DB.map((dbCat, i) => (
                          <option key={dbCat} value={dbCat}>{t.search.categories[i] ?? dbCat}</option>
                        ))}
                      </select>
                  ...
                  {/* Sıralama */}
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">{t.search.sort}</label>
                    <div className="relative">
                      <select
                        value={sortBy}
                      ...
                      >
                        {t.search.sortOptions.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                      </select>
                      <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-500 pointer-events-none" />
                    </div>
                  </div>
                  ...
                  {/* Başlangıç tarihi */}
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">{t.search.dateRange} ({t.common.dateFrom})</label>
                    <div className="relative">
                  ...
                  {/* Bitiş tarihi */}
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">{t.search.dateRange} ({t.common.dateTo})</label>
                    <div className="relative">
                      <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-500" />
                      <input
                        type="date"
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-800">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      {locationEnabled && selectedLocation.name
                        ? <><span className="font-medium">{selectedLocation.name}</span><span className="text-slate-500">({selectedLocation.radiusKm} {t.common.km})</span></>
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6 py-6">
          ...
          <div className="flex items-center gap-1 mb-6 p-1 rounded-2xl border border-slate-800 bg-slate-900/60 w-fit">
            {[
              { key: "all", label: t.search.all, count: filteredItems.length, color: "text-white" },
              { key: "lost", label: t.search.lost, count: lostItems.length, color: "text-amber-400", icon: <AlertCircle className="w-3.5 h-3.5" /> },
              { key: "found", label: t.search.found, count: foundItems.length, color: "text-emerald-400", icon: <CheckCircle2 className="w-3.5 h-3.5" /> },
            ].map((tab) => (
              <button
                key={tab.key}
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-900/40">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-slate-400">{t.search.lost} ({lostItems.length})</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="inline-block w-2.5 h-2.5 rounded-full bg-emerald-500" />
                    <span className="text-xs text-slate-400">{t.search.found} ({foundItems.length})</span>
                  </div>
                </div>
                <button
                  onClick={() => setIsFullMapOpen(true)}
            ...
            <>
              ...
              {visibleCount < displayedItems.length && (
                <div ref={sentinelRef} className="mt-8 flex justify-center">
                  <span className="text-sm text-slate-500">{t.search.loadMore}...</span>
                </div>
              )}
            </>
          )}
