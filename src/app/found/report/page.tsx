...
export default function FoundReportPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.error("Found item create error:", error);
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto max-w-2xl">
        ...
        <h1 className="mb-2 mt-4 text-4xl font-bold">Bulunan Eşya Bildir</h1>
        <p className="mb-8 text-slate-400">Bulduğun eşya için görselli ilan oluştur.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="Örn: Kahverengi cüzdan"
          ...
          <div>
            <label className="mb-2 block text-sm text-slate-300">Bulunduğu Konum</label>
          ...
          <div>
            <label className="mb-2 block text-sm text-slate-300">Bulunma Tarihi</label>
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="Bulduğun eşyanın detaylarını yaz..." rows={5}
              className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none" required />
          </div>
          ...
          <button type="submit" disabled={submitting}
            className="w-full rounded-xl bg-green-500 px-6 py-3 font-semibold text-white hover:bg-green-600 disabled:cursor-not-allowed disabled:opacity-60">
            {submitting ? "İlan oluşturuluyor..." : "Buluntu İlanı Oluştur"}
          </button>
        </form>
      </div>
